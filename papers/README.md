[English](./README.md) | [Français](./README.fr.md) | [日本語](./README.ja.md)

# Applied Papers

The present directory gathers those papers in which the Riemannian-geometric apparatus of Geometric Intelligence (GI) theory is brought to bear upon particular traditions of enquiry in the social sciences and the humanities. Its purpose is neither that of the `theory/` directory — which sets forth the mathematical foundations, culminating in the manifold-structure theorem for the image of a variational autoencoder decoder (Proposition 2.1) — nor that of the `companion/` directory, which furnishes such practical guidance as may assist the reader wishing to deploy the theory in the conduct of his own work. The rôle of `papers/` is, rather, that of the workshop: it is here that the instruments forged in `theory/` and assembled in `companion/` are put to use upon materials drawn from specific disciplines, and their capacities — no less than their limits — put to the test.

The reader who approaches GI theory for the first time will, as a rule, be better served by beginning with `theory/` or with `companion/`; the applied papers assume a familiarity with the basic vocabulary which those directories provide.

## Contents

### `quantitative_social_science/`

*A Riemannian Geometric Reconstruction of Quantitative Social Science: Lie Derivatives, Morse Theory, and a Geometric Answer to the Lucas Critique.*

A methodological proposal by which the principal analytical instruments of quantitative social science — the whole apparatus of econometrics, of computational political science, of quantitative sociology, and of cliometrics — may be reformulated within the framework of Riemannian geometry. Correspondences are established between ten existing methods (vector autoregression, Granger causality, the Chow test, simultaneous-equations models, the synthetic control method, the NOMINATE spatial model, Bayesian structural time-series models, network analysis, agent-based models, and the Lucas critique itself) and their counterparts on a Riemannian manifold constructed from the data by the pullback metric of a variational autoencoder decoder. The central contribution is a geometric restatement of the Lucas critique: the error, namely, of employing a fixed-metric model when the metric in question is not a Killing field — that is to say, when the Lie derivative of the metric along the policy vector field fails to vanish. The Lie derivative is offered, in consequence, as a measure of structural change free from the assumptions of any particular structural model, and as operating upon a plane altogether different from that of the DSGE programme.

Available in Japanese, in English, in French, and in Spanish.

### `sensibilities_annales_school/`

*The Geometry of Sensibilities: On the Connection between the History of Sensibilities of the Annales School and the Theory of Geometric Intelligence.*

A methodological proposal by which the Riemannian-geometric framework is applied to the *histoire des sensibilités* inaugurated by the third generation of the French Annales school — the olfactory history of Alain Corbin, the soundscape studies of the French countryside, the body-history of Georges Vigarello, the history of pain of Roselyne Rey. Three pathways are proposed for the *mise en variété* of qualitative sources: embedding by means of a language model; manual coding by the historian, in the tradition of Michel Vovelle; and the combination of the two. A proof of concept upon synthetic data — twelve quartiers of Paris across fifteen decadal periods from 1750 to 1900, with eight variables drawn from Corbin's own conceptual framework — establishes the computational feasibility of the method. The principal finding of the Lie-derivative analysis is that Haussmann's *grands travaux* produced a degree of olfactory structural transformation 2.74 times that of the construction of the sewers considered in isolation, and the Pasteurian revolution a factor of 1.75 — both results standing in accord with Corbin's thesis that olfactory experience is, in its essence, a socially constituted phenomenon.

Available in Japanese, in English, in German, and in French.

## A Note upon Future Additions

The directory `papers/` is intended to grow. Amongst the subjects at present in contemplation are the Tokugawa foundation of Edo, the Meiji Restoration, and further episodes in which the quantitative turn of GI theory may illuminate structures that qualitative scholarship has discerned without having measured. Further papers will be added as the applied programme develops.

## Licence

The papers in the present directory are released under the Creative Commons Attribution 4.0 International Licence (CC BY 4.0), in conformity with the remainder of the repository.

## Contact

etalecohomology2026@proton.me
